...
/*smart home system that automates the control of lighting and fan
...
#include"adc.h"
#include "pwm.h"
char temp; /*GLOBAL VARIABLE FOR TEMPERATURE */

/*function to display LDR sensor value and fan status (on/off)*/
void lcd_display(void)
{
	/*if condition to check on the fan status based on the temperature value*/
	if(temp>=25)
	{
...
{
	if(LM35_getTemperature()!=temp) /*check if temperature changed*/
		{
...
int main(void)
{
	...
	while(1)
	{
        ...
        {
        	fire_alert();
        	/* remains in alert mode until the flame is no longer detected.*/
        	while(FlameSensor_getValue()==1);
        	LCD_clearScreen();
        	Buzzer_off();
        	temp=-1; /*to restore temperature value before the fire*/
        }
